...
Layer::compatibleSurfaces(const GeometryContext& gctx, const Vector3& position,
  ...
  auto processSurface = [&](const Surface& sf, bool sensitive = false) {
    ...
    // Predicate selection (three regimes):
    //   (1) helixRegime + sensitive → closed-form helix-plane intersect
    //       (bounded check on the actual curved landing).
    //   (2) helixRegime + non-sensitive while moving inward → line
    //       predicate with pure −r̂ direction. Approach / boundary /
    //       layer-rep cylinders are radially well-conditioned in the
    //       barrel, so pure radial gives the cleanest intersect.
    //   (3) anything else (outward, non-barrel, no helix info) → line
    //       with the supplied raw tangent.
    ...
    } else if (helixRegime && inwardMotionLocal && rxyAtPos > 1.0e-6) {
